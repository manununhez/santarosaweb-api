<?php

namespace App\Admin\Controllers;

use App\ItemCategoryProduct;
use App\ItemCategory;
use App\Product;

use Encore\Admin\Controllers\AdminController;
use Encore\Admin\Form;
use Encore\Admin\Grid;
use Encore\Admin\Show;

class ItemCategoryProductController extends AdminController
{
    /**
     * Title for current resource.
     *
     * @var string
     */
    protected $title = 'Productos de una sub-categoria';

    /**
     * Make a grid builder.
     *
     * @return Grid
     */
    protected function grid()
    {
        $grid = new Grid(new ItemCategoryProduct());

        $grid->column('id', __('Id'));
        $grid->column('item_category_id', __('Item category id'));
        $grid->column('product_id', __('Product id'));
        $grid->column('created_at', __('Created at'));
        $grid->column('updated_at', __('Updated at'));

        return $grid;
    }

    /**
     * Make a show builder.
     *
     * @param mixed $id
     * @return Show
     */
    protected function detail($id)
    {
        $show = new Show(ItemCategoryProduct::findOrFail($id));

        $show->field('id', __('Id'));
        $show->field('item_category_id', __('Item category id'));
        $show->field('product_id', __('Product id'));
        $show->field('created_at', __('Created at'));
        $show->field('updated_at', __('Updated at'));

        return $show;
    }

    /**
     * Make a form builder.
     *
     * @return Form
     */
    protected function form()
    {
        $form = new Form(new ItemCategoryProduct());

        $form->select('item_category_id', __('Seleccionar la subCategoria'))->options(ItemCategory::all()->pluck('name', 'item_category_id'));
        $form->select('product_id', __('Seleccionar el producto'))->options(Product::all()->pluck('name', 'product_id'));

        return $form;
    }
}
